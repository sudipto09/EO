...
from prithvi_mae import PrithviMAE  # the file you already have
...
class TemporalDecoder(nn.Module):
    def __init__(self, embed_dim, num_timestamps=4, patch_size=16, img_size=224):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(embed_dim, 128, 4, stride=2, padding=1),  # 14→28
            nn.ReLU(),
...
def load_pipeline(device):
    ...
    # Prithvi-300M config (from the model card)
    model = PrithviMAE(
        img_size=224,
        patch_size=(1, 16, 16),
        num_frames=4,               # our 4 timestamps
        ...
        mlp_ratio=4.0,
        encoder_only=True,          # we only need the encoder
    )

    # load weights — strip 'encoder.' prefix if present
    checkpoint = torch.load(weights_path, map_location="cpu", weights_only=True)
    state_dict = checkpoint.get("model", checkpoint)  # handle wrapped checkpoints

    # try loading directly, fallback to stripping prefix
    try:
        ...
        model.load_state_dict(state_dict, strict=False)
        print("  Weights loaded with prefix strip ✓")
